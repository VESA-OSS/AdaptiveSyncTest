...
const float2 D5000White = float2(0.34567f, 0.35850f);  // D50
const float2 D6000White = float2(0.32168f, 0.33767);   // D60
const float2 D6500White = float2(0.31271f, 0.32902);   // D65  -assumed in Win10
...
const float3x3 XYZ_to_SMPTECRGB = float3x3(3.5053960, -1.0690722, 0.0563200, -1.7394894, 1.9778245, -0.1970226, -0.5439640, 0.0351722, 1.0502026);
const float3x3 XYZ_to_709RGB    = float3x3(3.2404542, -.9692660, 0.0556434, -1.5371585, 1.8760108, -.2040259, -0.4985314, 0.0415560, 1.0572252);
const float3x3 XYZ_to_AdobeRGB  = float3x3(2.0413690, -0.9692660, 0.0134474, -0.5649464, 1.8760108, -0.1183897, -0.3446944, 0.0415560, 1.0154096);
const float3x3 XYZ_to_BT2020RGB = float3x3(1.716650, -0.3556710, -0.2533660, -0.666684, 1.6164800, 0.0157681, 0.017640, -0.0427711, 0.9421030);
const float3x3 BT2020RGB_to_XYZ = float3x3(0.636958, 0.144617, 0.168881, 0.262700, 0.677998, 0.059302, 0.000000, 0.028073, 1.060985);
const float3x3 XYZtoLMS         = float3x3(0.400200, 0.707500, -0.080700, -0.228000, 1.150000, 0.061200, 0.000000, 0.000000, 0.918400);
const float3x3 LMStoXYZ         = float3x3(1.850243, -1.138302, 0.238435, 0.366831, 0.643885, -0.0106734, 0.000000, 0.000000, 1.088850);
...
static const float3x3 mat709to2020 = float3x3(0.6274040, 0.3292820, 0.0433136, 0.0690970, 0.9195400, 0.0113612, 0.0163916, 0.0880132, 0.8955950);

static const float3x3 mat2020to709 = float3x3(1.660491, -0.587640, -0.0728517, -0.124550, 1.132900, -0.0083480, -0.018151, -0.100579, 1.1187300);

static const float3x3 matRGBtoYCbCr = float3x3(0.299, -0.168736, 0.500, 0.587, -0.331264, -0.418688, 0.114, 0.500, -0.081312);

static const float3x3 matYCbCrtoRGB = float3x3(1.0, 1.0, 1.0, 0.0, -0.344136, 1.772, 1.402, -0.714136, 0.0);
...
{
    c = saturate(c);
    return float3(Apply2084(c.x), Apply2084(c.y), Apply2084(c.z));
}

float3 Remove2084(float3 c)
{
    return float3(Remove2084(c.x), Remove2084(c.y), Remove2084(c.z));
}

float3 Rec709ToRec2020(float3 color)  // assuming D65 white
{
    static const float3x3 conversion = {0.627402, 0.329292, 0.043306, 0.069095, 0.919544, 0.011360, 0.016394, 0.088028, 0.895578};
    return mul(conversion, color);
}

float3 Rec2020ToRec709(float3 color)  // assuming D65 white
{
    static const float3x3 conversion = {1.660496, -0.587656, -0.072840, -0.124547, 1.132895, -0.008348, -0.018154, -0.100597, 1.118751};
    return mul(conversion, color);
}

float3 RecDCIP3toRec2020(float3 color)  // assuming D65 white
{
    static const float3x3 conversion = {0.75383472, 0.19860256, 0.04756273, 0.04574290, 0.94178025, 0.01247684, -0.00121051, 0.01760467, 0.98360584};
    return mul(conversion, color);
}
...
float3 Rec2020toDCIP3(float3 color)  // assuming D65 white
{
    static const float3x3 conversion = {
        1.34357525, -0.28218550, -0.06138975, -0.06529575, 1.07578445, -0.01048870, 0.00282219, -0.01960177, 1.01677958};
    return mul(conversion, color);
}

float3 AdobeRGBtoRec2020(float3 color)  // assuming D65 white
{
    static const float3x3 conversion = {0.87733865, 0.07749581, 0.04516554, 0.09662091, 0.89153109, 0.01184800, 0.02292443, 0.04304395, 0.93403162};
    return mul(conversion, color);
}
...
{
    static const float3x3 conversion = {
        1.15197208, -0.09750475, -0.05446733, -0.12454710, 1.13289511, -0.00834801, -0.02253383, -0.04981527, 1.07234910};
    return mul(conversion, color);
}

float3 Rec709toDCIP3(float3 RGB709)
{
    static const float3x3 ConvMat = {0.822458, 0.177542, 0.000000, 0.033193, 0.966807, 0.000000, 0.017085, 0.072410, 0.910505};
    return mul(ConvMat, RGB709);
}

float3 DCIP3toRec709(float3 RGB709)
{
    static const float3x3 ConvMat = {1.224947, -0.224947, 0.000000, -0.042056, 1.042056, 0.000000, -0.019641, -0.078651, 1.098291};
    return mul(ConvMat, RGB709);
}
...
float3 XYZ_to_Luv(const float3& color_XYZ, const float3& white_XYZ)
{
    ...
    if (Y_ratio <= powf(delta, 3))
        ret.x = powf(2.0f / delta, 3) * Y_ratio;
    else
        ret.x = 116.0f * pow(Y_ratio, 1.0f / 3.0f) - 16.0f;
...
float3 Luv_to_XYZ(const float3& color_Luv, const float3& white_XYZ)
{
    ...
    if (color_Luv.x <= 8.0f)
        ret.y = white_XYZ.y * color_Luv.x * pow(delta / 2.0f, 3.0f);
    else
        ret.y = white_XYZ.y * pow((color_Luv.x + 16.0f) / 116.0f, 3.0f);
...
void PrintUsage()
{
    cout << "Usage: "
         << "gamut.exe"
         << " r_x r_y g_x g_y b_x b_y [w_x] [w_y]" << endl;
    ...
    for (int L = 0; L <= 100; L++)
    {
        for (int a = -200; a <= 200; a++)
        {
            for (int b = -200; b <= 200; b++)
            {
                ...
                if (testColor_RGB.x < 0.0f || testColor_RGB.x > 1.0f)
                    continue;
                if (testColor_RGB.y < 0.0f || testColor_RGB.y > 1.0f)
                    continue;
                if (testColor_RGB.z < 0.0f || testColor_RGB.z > 1.0f)
                    continue;
                hits++;
            }
        }
